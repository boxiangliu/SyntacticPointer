#!/usr/bin/env bash
CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=4 python -u parsing.py --mode train --config configs/parsing/l2r.json --num_epochs 600 --batch_size 32 \
 ...
 --word_embedding sskip --word_path "embs/sskip.eng.100.gz" --char_embedding random \
 --punctuation '.' '``' "''" ':' ',' \
 --train "data/ptb-train.conllx" \
 --dev "data/ptb-dev.conllx" \
 --test "data/ptb-test.conllx" \
 --model_path "models/l2r"$1
