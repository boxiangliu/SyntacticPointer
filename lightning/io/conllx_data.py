...
def create_alphabets(
        ...
        for data_path in data_paths:
            with open(data_path, "r") as file:
                for line in file:
                    ...
                    if word not in vocab_set and (
                        word in embedd_dict or word.lower() in embedd_dict
                    ):
                        vocab_set.add(word)
    ...
    if not os.path.isdir(alphabet_directory):
        ...
        # collect singletons
        singletons = set(
            [word for word, count in vocab.items() if count <= min_occurance]
        )
        ...
        vocab_list = [
            ...
            if word in _START_VOCAB or vocab[word] > min_occurance
        ]
        logger.info(
            "Total vocabulary size (w.o. rare word): {}".format(len(vocab_list))
        )
    ...
    logger.info(
        "Word Alphabet Size (Singleton): %d (%d)"
